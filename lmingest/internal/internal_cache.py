...
from lmingest import ApiClient
...
class BatchingCache(object):
  ...
  def __init__(self, api_client, interval, batch, response_callback, request_cb,
    ...
    if not response_callback and not isinstance(response_callback,
                                                LMResonseInterface):
      logger.error("Response callback is not defined or valid.")
  ...
  def response_handler(self, request, response, status, headers, reason=None):
