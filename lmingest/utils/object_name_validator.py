...
PATTERN_INVALID_DEVICE_DISPLAY_NAME = re.compile(REGEX_INVALID_DEVICE_DISPLAY_NAME)
...
PATTERN_INVALID_DATA_SOURCE_DISPLAY_NAME = re.compile(REGEX_INVALID_DATA_SOURCE_DISPLAY_NAME)
...
class ObjectNameValidator:
    ...
    * "(" ")" for this name
    ...
    def validate_datasource_name(self, name):
        ...
        return self._validate(name, "datasource name", PATTERN_INVALID_DATA_SOURCE_NAME)
    ...
    def validate_device_display_name(self, name):
        return self._validate(name, "resource name", PATTERN_INVALID_DEVICE_DISPLAY_NAME)
    ...
    def is_valid_datasource_display_name(self, name):
        return bool(self.validate_datasource_display_name(name))  # Strings.isNullOrEmpty(validateDataSourceDisplayName(name));
    ...
    def validate_datasource_display_name(self, name):
        ...
        return self._validate(name, "datasource display name", PATTERN_INVALID_DATA_SOURCE_DISPLAY_NAME)
            ...
            if len(resource_ids) == 0:
                ...
                for key, value in resource_ids.items():
                    ...
                    if not self.pass_empty_and_space_check(value):
                        ...
                        err_msg += "Invalid resource Id Value : %s for Key : %s." % (value, key)
        return err_msg
        ...
        if resource_properties is not None:
            if len(resource_properties) > 0:
                for key, value in resource_properties.items():
                    ...
                    elif key.lower().startswith("system.") or key.lower().startswith("auto."):
                    ...
                    if not self.pass_empty_and_space_check(value):
                        ...
                        err_msg += "Invalid resource Property Value : %s for Key : %s." % (value, key)
        return err_msg
            ...
            if len(instance_properties) > 0:
                for key, value in instance_properties.items():
                    ...
                    if not self.pass_empty_and_space_check(value):
                        ...
                        err_msg += "Invalid instance Property Value : %s for Key : %s." % (key, value)
    ...
    def check_datapoint_type_validation(self, data_point_type):
        ...
        if data_point_type is not None:
            ...
            if data_point_type not in data_point_types:
                err_msg += "The datapoint %s is having Invalid dataPointType %s." % (self.name, data_point_type)
        return err_msg

    def check_datapoint_aggregation_type_validation(self, data_point_aggregation_type):
        err_msg = ""
        ...
        if data_point_aggregation_type is not None:
            ...
            if data_point_aggregation_type not in data_point_aggregation_types:
                err_msg += "The datapoint %s is having Invalid data_point_aggregation_type %s." % (self.name, data_point_aggregation_type)
